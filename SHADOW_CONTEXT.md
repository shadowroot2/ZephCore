...
- BLE bridge M6 868 ↔ Heltec V3 433 подтверждён пользователем как рабочий
  (2026-08-15): статус `connected`, `bridge ping` отвечает 69–100 ms. Для
  этой пары M6 `9E:E8:62:0B:0D:D5` (random), Heltec
  `76:79:3A:43:CA:48` (public). После смены peer нужно указать адрес именно
...
- Батарея: AIN3 / `P0.05`, 12-bit SAADC, `GAIN_1_6` + internal ref.
  Номинальный множитель Meshtastic `6300` (делитель 1.75:1) на тестовой M3
  показывал 4.47 V. В ZephCore задан `5920`, что при том же raw даёт ~4.20 V;
  это калибровка напряжения, не кривая процента. Она также ожидает аппаратной
  проверки после новой сборки.
...
- Индикация зарядки M3 подтверждена пользователем 2026-08-21:
  ...
  питания: на этой ревизии остаётся активным после отключения кабеля. Полный
  заряд — только по `P0.24/DONE`, стабильно 20 с при активном P0.31; ADC не
  использовать для выбора зелёного, так как при зарядке он даёт ложные 100%.
...
- Последняя прошивка: `firmware/thinknode_m3-companion-2026-08-21.uf2`
  (UF2, 715776 B), содержит питание AHT10, его рабочую последовательность
  Meshtastic, множитель батареи `5920` и подтверждённую логику зарядки P0.31.
...
- Последние подтверждённые образы:
  ...
  `firmware/t1000_e-companion-2026-08-21-fall-sos-switch.uf2`.
